...
namespace OpenRA.Mods.AS.Traits
{
	...
	public class BaseBuilderBotASModuleInfo : ConditionalTraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new BaseBuilderBotASModule(init.Self, this); }
	}

	public class BaseBuilderBotASModule : ConditionalTrait<BaseBuilderBotASModuleInfo>, IBotTick, IBotPositionsUpdated, IBotRespondToAttack, IBotRequestPauseUnitProduction
	{
		...
		List<BaseBuilderQueueManagerAS> builders = new List<BaseBuilderQueueManagerAS>();

		public BaseBuilderBotASModule(Actor self, BaseBuilderBotASModuleInfo info)
			: base(info)
		{
			world = self.World;
			player = self.Owner;
		...
		protected override void TraitEnabled(Actor self)
		{
			...
			foreach (var building in Info.BuildingQueues)
				builders.Add(new BaseBuilderQueueManagerAS(this, building, player, playerPower, playerResources, resourceTypeIndices));
			foreach (var defense in Info.DefenseQueues)
				builders.Add(new BaseBuilderQueueManagerAS(this, defense, player, playerPower, playerResources, resourceTypeIndices));
		}
